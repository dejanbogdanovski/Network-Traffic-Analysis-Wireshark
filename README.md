...
## Environment & Tools
* **Wireshark:** Used for deep packet inspection and traffic flow analysis.
* **PowerShell:** Employed for cryptographic hashing (SHA256) of extracted malware components.
* **Analysis Focus:** DNS anomalies, HTTP object extraction, and PowerShell script deconstruction.
...
Identified a massive surge in DNS "No such name" errors (383 total), signaling automated malicious activity or domain hunting.
...
Extracted the DNS "Answer" section, confirming that the fraudulent domain resolved to the attacker's command-and-control (C2) infrastructure at `82.221.136.26`.
...
Isolated and exported several suspicious objects from the traffic, including `TeamViewer`, a PowerShell script `29842.ps1`, and a mysterious file named `TV`.
![HTTP Payloads](<Network Traffic Analysis Wireshark/06_HTTP_Payloads.png>)

**Step 7: Script Deconstruction**
Analyzed the TCP stream of the PowerShell script, which revealed functions for downloading additional payloads and establishing persistence.
...
Deep dive into the `TV` file revealed it was a Windows executable/DLL (identified by the **MZ** header) disguised as a generic data stream.
...
* **Calculated Hash:** `3448DA03808F24568E6181011F8521C0713EA6160EFD05BFF20C43B091FF59F7`
